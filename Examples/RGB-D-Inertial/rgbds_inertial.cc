...
int main(int argc, char **argv)
{
    ...
    if (vstrImageFilenamesRGB.empty() || vTimestampsImu.empty())
    {
        cerr << endl
             << "ERROR: Failed to load images or IMU for sequence." << endl;
        return 1;
    }
    ...
    int first_imu = 0;
    while (first_imu < static_cast<int>(vTimestampsImu.size()) && vTimestampsImu[first_imu] <= vTimestamps[0])
        first_imu++;
    ...
    for (int ni = 0; ni < nImages; ni++)
    {
        ...
        if (ni > 0)
        {
            while (first_imu < static_cast<int>(vTimestampsImu.size()) && vTimestampsImu[first_imu] <= tframe)
            {
            ...
            // printf("Frame %d: Read %lu IMU measurements.\n", ni, vImuMeas.size());
        }
