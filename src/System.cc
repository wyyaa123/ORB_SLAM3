...
namespace ORB_SLAM3
{
    ...
    Sophus::SE3f System::TrackRGBD(const cv::Mat &im, const cv::Mat &depthmap, const cv::Mat &sem, const double &timestamp, const vector<IMU::Point> &vImuMeas, string filename)
    {
        if (mSensor != RGBD && mSensor != IMU_RGBD)
        {
            cerr << "ERROR: you called TrackRGBD but input sensor was not set to RGBD." << endl;
        ...
        if (settings_ && settings_->needToResize())
        {
            ...
            cv::resize(sem, imSemToFeed, settings_->newImSize());
        }
        ...
        Sophus::SE3f Tcw = mpTracker->GrabImageRGBD(imToFeed, imDepthToFeed, imSemToFeed, timestamp, filename);
