...
namespace ORB_SLAM3
{
    ...
    KeyFrame::KeyFrame(Frame &F, Map *pMap, KeyFrameDatabase *pKFDB) : bImu(pMap->isImuInitialized()), mnFrameId(F.mnId), mTimeStamp(F.mTimeStamp), mnGridCols(FRAME_GRID_COLS), mnGridRows(FRAME_GRID_ROWS),
                                                                       ...
                                                                       mbHasVelocity(false), mImgLeft(F.mImgLeft.clone()), mvBezierCurves(F.mvBezierCurves), mvpMapCurves(F.mvpMapBeziers)
    {
        mnId = nNextId++;
    ...
    void KeyFrame::SetBadFlag()
    {
        ...
        for (int i = 0; i < mvpMapCurves.size(); i++)
        {
            if (mvpMapCurves[i])
            {
                mvpMapCurves[i]->EraseObservation(this);
            }
        }
    ...
    void KeyFrame::AddMapCurve(MapCurve *pMB, const size_t &idx)
    {
        unique_lock<mutex> lock(mMutexFeatures);
        mvpMapCurves[idx] = pMB;
    }

    void KeyFrame::EraseMapCurveMatch(const int &idx)
    {
        unique_lock<mutex> lock(mMutexFeatures);
        mvpMapCurves[idx] = static_cast<MapCurve *>(NULL);
    }

    void KeyFrame::EraseMapCurveMatch(MapCurve *pMB)
    {
        unique_lock<mutex> lock(mMutexFeatures);
        for (size_t i = 0, iend = mvpMapCurves.size(); i < iend; ++i)
        {
            if (mvpMapCurves[i] == pMB)
                mvpMapCurves[i] = static_cast<MapCurve *>(NULL);
        }
    }

    void KeyFrame::ReplaceMapCurveMatch(const int &idx, MapCurve *pMB)
    {
        unique_lock<mutex> lock(mMutexFeatures);
        mvpMapCurves[idx] = pMB;
    }

    void KeyFrame::ReplaceMapCurveMatch(MapCurve *pOldMB, MapCurve *pNewMB)
    {
        unique_lock<mutex> lock(mMutexFeatures);
        for (size_t i = 0, iend = mvpMapCurves.size(); i < iend; ++i)
        {
            if (mvpMapCurves[i] == pOldMB)
                mvpMapCurves[i] = pNewMB;
        }
    }

    std::set<MapCurve *> KeyFrame::GetMapCurves()
    {
        unique_lock<mutex> lock(mMutexFeatures);
        set<MapCurve *> s;
        for (size_t i = 0, iend = mvpMapCurves.size(); i < iend; i++)
    ...
    int KeyFrame::TrackedMapCurves(const int &minObs)
    {
        unique_lock<mutex> lock(mMutexFeatures);
        ...
        for (int i = 0; i < mvpMapCurves.size(); i++)
        {
            ...
            if (pMB)
            {
                if (!pMB->isBad())
                {
                    if (bCheckObs)
                    {
                        if (mvpMapCurves[i]->Observations() >= minObs)
                            nCurves++;
                    }
                    else
                        nCurves++;
    ...
    MapCurve *KeyFrame::GetMapCurve(size_t idx)
    {
        unique_lock<mutex> lock(mMutexFeatures);
        return mvpMapCurves[idx];
    }
