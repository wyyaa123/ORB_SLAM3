...
namespace ORB_SLAM3
{
    ...
    cv::Mat FrameDrawer::DrawFrameEdges()
    {
        ...
        if (mvEdges.empty()) return cv::Mat();
    ...
    cv::Mat FrameDrawer::DrawFrameSemanticMask(float imageScale)
    {
        unique_lock<mutex> lock(mMutex);
        ...
        cv::Mat sem = mSemMask.clone();
        if (imageScale != 1.f)
        {
            int imWidth = sem.cols / imageScale;
            int imHeight = sem.rows / imageScale;
            cv::resize(sem, sem, cv::Size(imWidth, imHeight), 0, 0, cv::INTER_NEAREST);
        }

        if (sem.channels() != 1)
        {
            cv::Mat gray;
            cv::cvtColor(sem, gray, cv::COLOR_BGR2GRAY);
            sem = gray;
        }

        cv::Mat colorMask = cv::Mat::zeros(sem.size(), CV_8UC3);
        static const cv::Vec3b kColors[] = {
            cv::Vec3b(0, 0, 0),
            cv::Vec3b(0, 114, 189),
            cv::Vec3b(217, 83, 25),
            cv::Vec3b(237, 177, 32),
        ...
        for (int cls = 1; cls < 10; ++cls)
            colorMask.setTo(kColors[cls], sem == cls);

        colorMask.setTo(cv::Vec3b(255, 255, 255), sem == 255);

        return colorMask;
    }

    cv::Mat FrameDrawer::DrawKeyFrameEdges()
    {
        unique_lock<mutex> lock(mMutex);

        if (mvKfEdges.empty()) return cv::Mat();

        cv::RNG rng(66);
        cv::Mat image_viz = mKfImg.clone();
        ...
        cv::cvtColor(image_viz, image_viz, cv::COLOR_GRAY2BGR);
        int maxLabel = 0;
        for (int i = 0; i < mvKfEdges.size(); ++i)
        {
            int b = rng.uniform(0, 255);
            int g = rng.uniform(0, 255);
            int r = rng.uniform(0, 255);
            cv::Vec3b color = cv::Vec3b(b, g, r);
            // if(mvEdgeClusters[i].mvPoints.size()<15) continue;
            for (int j = 0; j < mvKfEdges[i].mvPoints.size(); ++j)
            {
                orderedEdgePoint curr = mvKfEdges[i].mvPoints[j];
                image_viz.at<cv::Vec3b>(curr.y, curr.x) = color;
                cv::circle(image_viz, cv::Point(curr.x, curr.y), 1, color, 1, cv::LINE_AA);
            }
        }
    ...
    void FrameDrawer::Update(Tracking *pTracker)
    {
        ...
        KeyFrame *pReferenceKF = pTracker->mCurrentFrame.mpReferenceKF;
        if (pReferenceKF && !pReferenceKF->mImg.empty())
            pReferenceKF->mImg.copyTo(mKfImg);
        else
            mKfImg.release();
